...
const thinLineAssist = document.querySelector('#thin-line-assist');
const repairStrength = document.querySelector('#repair-strength');
...
button.addEventListener('click', async () => { const file = fileInput.files[0]; if (!file) { error.textContent = 'PNGまたはJPEG画像を選択してください。'; return; } error.textContent = ''; button.disabled = true; loading.hidden = false; const form = new FormData(); form.append('image', file); form.append('threshold', range.value); form.append('line_color', lineColor.value); form.append('thin_line_assist', thinLineAssist.checked ? 'true' : 'false'); form.append('repair_strength', repairStrength.value); try { const response = await fetch('/api/extract', { method: 'POST', body: form }); if (!response.ok) { const body = await response.json().catch(() => ({})); throw new Error(body.detail || '線画化に失敗しました。'); } const blob = await response.blob(); if (resultUrl) URL.revokeObjectURL(resultUrl); resultUrl = URL.createObjectURL(blob); result.src = resultUrl; download.href = resultUrl; download.hidden = false; } catch (exception) { error.textContent = exception.message; } finally { button.disabled = false; loading.hidden = true; } });
